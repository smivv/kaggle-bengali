...
def pairwise_distance_torch(embeddings):
    ...
    pairwise_distances_squared = torch.max(pairwise_distances_squared, torch.tensor([0.]))
...
class TripletLoss(nn.Module):
    ...
    def forward(self, y_true, y_pred, device, margin=1.0):
        """
       ...
       """
        ...
        mask_positives = adjacency.to(dtype=torch.float32) - torch.diag(torch.ones(batch_size))
        num_positives = mask_positives.sum()

        triplet_loss = (torch.max(torch.mul(loss_mat, mask_positives), torch.tensor([0.]))).sum() / num_positives
